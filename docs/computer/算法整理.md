...
由于循环执行次数最多的是第 4、5 行代码， 这两行代码被执行了 n 次，所以总的时间复杂度就是 O(n)
...
快速排序在最糟糕的情况下，也就是说取数每次都无法平分数组， 这种情况算法复杂度为O(n2)
...
### 堆栈Stack FILO(first-in-last-out)
